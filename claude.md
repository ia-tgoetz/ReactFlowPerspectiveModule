...
4. **Waypoints are stored lazily — empty on first connect, computed on first segment drag or reconnect.** `onConnect` writes `waypoints: []`; `CustomEdge` routes live via `computeAutoWaypoints` until the user drags a segment, which persists the waypoints. `onEdgeUpdate` (endpoint reconnect) still calls `getHandlePixelPos` + `computeAutoWaypoints` immediately so a moved endpoint always gets a clean route.
