...
const CustomEdge = ({ sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, data, markerEnd, style, label }: any) => {
    ...
    if (data?.lineType === 'step' || data?.lineType === 'smoothstep' || !data?.lineType) {
        ...
        [edgePath, labelX, labelY] = getSmoothStepPath({ 
            ...
            borderRadius: data?.lineType === 'step' ? 0 : 15, 
            centerX: defaultCenterX + offsetX, centerY: defaultCenterY + offsetY
        });
...
const mapIgnitionToReactFlowNodes = (ignitionNodes: any, handleGearClick: (id: string) => void, selectedId: string | null, globalHideHandles: boolean) => {
    ...
    return Object.entries(ignitionNodes).filter(([id, nodeData]: any) => nodeData !== null && nodeData !== undefined).map(([id, nodeData]: any) => { return { id, type: 'architecture', selected: id === selectedId, position: { x: nodeData.x || 0, y: nodeData.y || 0 }, data: { label: nodeData.label || 'Unknown', svg: nodeData.svg || '', tooltip: nodeData.tooltip || '', configs: nodeData.configs || {}, style: nodeData.style || {}, paletteId: nodeData.paletteId || 'unknown', hideHandles: nodeData.hideHandles, globalHideHandles: globalHideHandles, onGearClick: handleGearClick } }; });
};
...
export interface ArchitectureBuilderProps {
    hideHandles?: any; snapEnabled?: any; snapPixels?: any;
...
export const ArchitectureBuilder = observer((props: ComponentProps<ArchitectureBuilderProps>) => {
  ...
  React.useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
        ...
        if ((e.ctrlKey || e.metaKey) && e.key === 'c') {
            if (selectedId && rawNodesDict[selectedId]) clipboardRef.current = rawNodesDict[selectedId];
        }
        
        if ((e.ctrlKey || e.metaKey) && e.key === 'v') {
            const copiedNode = clipboardRef.current;
            if (copiedNode && props.store?.props) {
                let dropX = copiedNode.x + (snapEnabled ? snapPixels * 2 : 30);
                let dropY = copiedNode.y + (snapEnabled ? snapPixels * 2 : 30);
                const newNodeId = generateShortId();
                const nextNodes = { ...rawNodesDict };
                const newNode = JSON.parse(JSON.stringify({ ...copiedNode, x: dropX, y: dropY }));
  ...
  }, [selectedId, rawNodesDict, snapEnabled, snapPixels, props.store]);
  ...
  const isValidConnection = React.useCallback((connection: any) => {
      const validTypes = getValidIntersection(connection.source, connection.target, updatingEdgeRef.current || undefined);
  ...
  const onNodeContextMenu = React.useCallback((event: any, node: any) => {
      ...
      if (bounds) { setContextMenu({ id: node.id, top: event.clientY - bounds.top, left: event.clientX - bounds.left, type: 'node' }); setActiveSubMenu(null); }
  }, []);
  ...
  const handleContextMenuAction = (action: string) => {
      ...
      if (action === 'paste' && contextMenu.type === 'pane') {
          const copiedNode = clipboardRef.current;
          if (copiedNode && reactFlowInstance && contextMenu.clientX && contextMenu.clientY) {
              ...
              if (props.store?.props) {
                  const newNodeId = generateShortId();
                  const nextNodes = { ...rawNodesDict };
                  nextNodes[newNodeId] = JSON.parse(JSON.stringify({ ...copiedNode, x: dropX, y: dropY }));
                  props.store.props.write('nodes', nextNodes);
                  setSelectedId(newNodeId);
              }
          }
              ...
              if (props.store?.props) {
                  ...
                  Object.keys(nextEdges).forEach(edgeId => {
                      if (nextEdges[edgeId].source === contextMenu.id || nextEdges[edgeId].target === contextMenu.id) {
                          delete nextEdges[edgeId]; edgesChanged = true;
                      }
                  });
                  props.store.props.write('nodes', nextNodes);
                  if (edgesChanged) props.store.props.write('edges', nextEdges);
  ...
  const onDrop = React.useCallback((event: any) => {
    ...
    if (props.store?.props) {
        const newNodeId = generateShortId();
        const newNodeData = { paletteId: paletteItem.id, label: paletteItem.label, svg: paletteItem.svg, tooltip: paletteItem.tooltip, x: dropX, y: dropY, hideHandles: false, style: paletteItem.style || { classes: "" }, configs: initialConfigs, supportedConnections: paletteItem.supportedConnections || [] };
  ...
  const onNodeDragStop = React.useCallback((event: any, node: any) => {
    if (props.store?.props) { const nextNodes = { ...rawNodesDict }; if (!nextNodes[node.id]) return; nextNodes[node.id] = { ...nextNodes[node.id], x: Math.round(node.position.x), y: Math.round(node.position.y) }; props.store.props.write('nodes', nextNodes); }
            ...
            <div style={{ position: 'absolute', top: contextMenu.top, left: contextMenu.left, zIndex: 10, backgroundColor: 'var(--neutral-20)', border: '1px solid var(--neutral-50)', borderRadius: '4px', boxShadow: '0 4px 6px rgba(0,0,0,0.3)', padding: '4px', minWidth: '140px', fontSize: '12px' }}>
                ...
                {contextMenu.type !== 'pane' && (
                    <>
                        ...
                        <div style={{ padding: '5px 8px', cursor: 'pointer', color: 'var(--error)', borderTop: '1px solid var(--neutral-40)' }} onMouseEnter={() => setActiveSubMenu(null)} onClick={() => handleContextMenuAction('delete')}>🗑️ Delete</div>
                    </>
                )}
            </div>
        )}
